...
const incidents: Incident[] = [
  {
    ...
    site: 'client',
    details:
      'Temporary incident caused by a tracking (analytics) service that could block the site from loading in certain cases.',
  },
  ...
  return (
    <Page width="sm" title={m.status_.title}>
      ...
      <Box sx={{mt: 1, display: 'grid', gap: 1, gridTemplateColumns: {xs: '1fr', sm: '1fr 1fr'}}}>
        <HistoryTrackingCard
          HistoryTrackingProps={{incidents: incidents.filter(_ => _.site === 'api')}}
          site="api"
          head={m.status_.apiRequests}
        />
        <HistoryTrackingCard
          HistoryTrackingProps={{incidents: incidents.filter(_ => _.site === 'pyx-server')}}
          site="pyx-server"
          head={m.status_.xlsFormManager}
        />
        <HistoryTrackingCard
          HistoryTrackingProps={{incidents: incidents.filter(_ => _.site === 'db')}}
          site="db"
          head={m.status_.databases}
        />
        <HistoryTrackingCard
          HistoryTrackingProps={{incidents: incidents.filter(_ => _.site === 'client')}}
          site="client"
          head={m.status_.client}
        />
      </Box>
    </Page>
  )
  ...
  return (
    <Card>
      <CardContent>
        <Typography
          fontWeight="500"
          sx={{mb: 1, display: 'flex', alignItems: 'center', gap: 1, justifyContent: 'space-between'}}
        >
          {head}
          <Tooltip title={m.status_[AppStatus.ok].title}>
...
function HistoryTracking({maxDays = 60, sx, incidents = [], ...props}: HistoryTrackingProps) {
  ...
  const {days, uptimePercent} = useMemo(() => {
    const daysData = mapFor(maxDays, i => {
      ...
      const dayIncidents = incidents.filter(inc => isBefore(inc.start, dayEnd) && isAfter(inc.end, dayStart))
                  ...
                  <ul style={{margin: 0, paddingLeft: '1.2rem', fontSize: '0.75rem'}}>
                    {day.incidents.map((inc, idx) => (
                      <Box
                        component="li"
                        key={idx}
                        sx={{'::marker': {color: inc.severity === 'critical' ? 'error.main' : 'warning.main'}}}
                      >
                        <strong>
                          {format(inc.start, 'HH:mm')} - {format(inc.end, 'HH:mm')}
                        </strong>{' '}
                        {inc.details}
                      </Box>
                    ))}
